...
static void dhcps_send_offer()
{
  ...
  add_offer_options(add_msg_type(&dhcp_message_repository->options[4], DHCP_MESSAGE_TYPE_OFFER));
	...
	sendto(DHCPs_SOCKET, (uint8_t *)dhcp_message_repository, sizeof(dhcps_msg), (uint8_t *)&dhcps_send_broadcast_address.addr, DHCP_CLIENT_PORT);
}
...
//	printf("%s(%d)\r\n", __FILE__, __LINE__);
	if(getSn_SR(DHCPs_SOCKET) != SOCK_UDP)
	   socket(DHCPs_SOCKET, Sn_MR_UDP, DHCP_SERVER_PORT, 0x00);

  if((len = getSn_RX_RSR(DHCPs_SOCKET)) > 0)
  {
    len = recvfrom(DHCPs_SOCKET, (uint8_t *)dhcp_message_repository, len, client_addr, &client_port);
