...
//#define DEBUG_SNTPS_RUN
  ...
  uint32_t recv_frac;
	...
	if(getSn_SR(NTPs_SOCKET) != SOCK_UDP)
	   socket(NTPs_SOCKET, Sn_MR_UDP, ntp_port, 0x00);
  	...
  	case SOCK_UDP:
  		if ((RSR_len = getSn_RX_RSR(NTPs_SOCKET)) > 0)
  		{
  		  set_timestamp(&recv_sec, &recv_frac);
  			...
  			recvfrom(NTPs_SOCKET, data_buf, RSR_len, (uint8_t *)&destip, &destport);
        ...
        sendto(NTPs_SOCKET, (uint8_t*)&NTPsformat, sizeof(NTPsformat), destip, destport);
  		...
  		break;
  	case SOCK_CLOSED:
  		socket(NTPs_SOCKET,Sn_MR_UDP,ntp_port,0);
  		break;
	}
	// Return value
	// 0 - failed / 1 - success
